 ...
 * @copyright Copyright © 2010-2024, 2026 Gustaf Mossakowski
...
function mf_media_mediapool_view($vars, $parameter) {
	...
	if (reset($full_path_parts) === '-tags') {
		...
		array_shift($full_path_parts);
		$view['tag'] = implode('/', $full_path_parts);
...
function mf_media_tools($view) {
	...
	$tools[] = [
		'img' => 'list-ul',
		'alt' => wrap_text('Gallery'),
		'title' => wrap_text('Display as Gallery'),
		'link' => $view['type'] === 'tree' ? '../' : ''
	];
	$tools[] = [
		'img' => 'list-table',
		'alt' => wrap_text('Table'),
		'title' => wrap_text('Display as Table'),
		'link' => $view['type'] === 'tree' ? '' : '-/'
	];
	if ($path = wrap_path('media_download', $view['tag_path'] ?? $view['full_path'] ?? '-media', ['hide_missing' => true])) {
		...
		$tools[] = [
			'img' => 'download',
			'alt' => wrap_text('Download'),
			'title' => wrap_text('Download files in ZIP archive'),
			'link' => $path
		];
	}
